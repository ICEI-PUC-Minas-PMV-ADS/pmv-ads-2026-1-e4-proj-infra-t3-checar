 ...
 *               email:
 *                 type: string
 *                 example: usuario@checar.com
 *               senha:
 *                 type: string
 *                 example: "123456"
 *     responses:
 *       200:
 *         description: Login realizado com sucesso e conexão com banco ativa
 *       401:
 *         description: E-mail ou senha incorretos
 *       500:
 *         description: Erro de conexão com o MongoDB
 */
router.post('/login', async (req, res) => {
    try {
        const { email, senha } = req.body;

        // Aqui você deve importar seu Model de Usuario (ex: const Usuario = require('./models/Usuario'))
        // Por enquanto, vamos simular a busca para testar a rota no Swagger:
        ...
        res.status(500).json({ erro: "Erro no banco de dados", detalhe: error.message });
    }
});
