...
router.post("/checklists", async (req, res) => {
  try {
    ...
    console.error('[POST /checklists]', error.name, error.message, error.stack);
    res.status(500).json({ erro: 'Erro interno ao criar checklist.' });
  }
});
...
router.get("/checklists/historico", async (req, res) => {
  try {
    ...
    console.error('[GET /checklists/historico]', error.name, error.message, error.stack);
    res.status(500).json({ erro: 'Erro ao buscar histórico de checklists.' });
  }
});
...
router.get("/checklists/:id", async (req, res) => {
  try {
    ...
    console.error('[GET /checklists/:id]', error.name, error.message, error.stack);
    res.status(500).json({ erro: 'Erro interno ao buscar checklist.' });
  }
});
...
router.put("/checklists/:id", async (req, res) => {
  try {
    ...
    console.error('[PUT /checklists/:id]', error.name, error.message, error.stack);
    res.status(500).json({ erro: 'Erro interno ao atualizar checklist.' });
  }
});
...
router.delete("/checklists/:id", async (req, res) => {
  try {
    ...
    console.error('[DELETE /checklists/:id]', error.name, error.message, error.stack);
    res.status(500).json({ erro: 'Erro interno ao deletar checklist.' });
  }
});
