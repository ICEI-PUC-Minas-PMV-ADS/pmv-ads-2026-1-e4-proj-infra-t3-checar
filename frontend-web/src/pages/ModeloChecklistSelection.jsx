  ...
  useEffect(() => {
    ...
    const carregarDados = async () => {
      try {
        ...
        const [modelosResponse, vehicleResponse] = await Promise.all([
          axios.get('/api/modelochecklists', { params: { ativo: 'true' } }),
          vehicleId ? axios.get(`/api/vehicles/${vehicleId}`) : Promise.resolve(null),
        ]);

        if (!mounted) return;

        setModelos(modelosResponse.data || []);
        setVehicle(vehicleResponse?.data?.data || vehicleResponse?.data || null);
      } catch (err) {
        if (mounted) setError(`Falha ao carregar modelos: ${err.response?.data?.erro || err.message}`);
      } finally {
    ...
    <section className="mx-auto flex min-h-[calc(100vh-7rem)] max-w-4xl flex-col gap-6">
      <header className="border-b border-white/10 pb-5">
        ...
        <h1 className="text-3xl font-bold tracking-normal">
          {isChecklistFlow ? 'Escolha o' : 'Modelos'} <span className="text-[#00b4d8]">{isChecklistFlow ? 'Modelo' : 'de Checklist'}</span>
        </h1>
      ...
      {error && <p className="rounded-lg border border-red-500/50 bg-red-950/40 px-4 py-3 text-red-100">{error}</p>}

      {loading ? (
        <div className="flex min-h-48 items-center justify-center text-white/70">Carregando modelos...</div>
      ) : (
