...
export default function UploadFotos({ aoVoltar }) {
  ...
  const cadastrarVeiculo = async () => {
    ...
    const response = await axios.post('http://192.168.15.2:3000/vehicles', vehicleData);
  ...
  const enviarFotos = async (placa) => {
    ...
    const response = await axios.post('http://192.168.15.2:3000/inspecao/upload', formData, {
