...
import { View, Text, ScrollView, SafeAreaView, StatusBar, TouchableOpacity, StyleSheet, Alert, Modal, ActivityIndicator } from 'react-native';
...
export default function VehicleList({ navigation }) {
  ...
  const handleDeleteVehicle = (id, plate) => {
    Alert.alert(
      'Confirmar Deleção',
      `Tem certeza que deseja deletar o veículo ${plate}?`,
      [
        {
          text: 'Cancelar',
          onPress: () => {},
        ...
        },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Erro', `Falha ao deletar veículo: ${err.message}`);
            }
          },
          style: 'destructive',
        },
